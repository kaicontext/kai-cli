...
TESTS, INTENT AND CONSISTENCY ARE NOT DEFECTS BY THEMSELVES. "This path has no test", "the test never asserts X", "this differs from the other handlers", a name, a missing method guard nothing reaches, and a behaviour change the PR says it makes on purpose are not ISSUES. Report one only when it names a specific missing requirement — a behaviour this change must guarantee (usually the fix it claims) that nothing verifies — or a concrete regression risk: what breaks, for whom, on which input. Otherwise leave it out, or say it in one sentence of prose. On the 2026-09-24 benchmark these took over the defect list: Keycloak #36880 ("the test never asserts an admin with MANAGE_CLIENTS is still allowed"), Cal.com #11059 ("inconsistent with every other handler in apps/web/pages/api"), Cal.com #10600 ("minLength/maxLength are native attributes, not zod validation"). An intentional change the author described is at most a DECISION, and only when it reaches a customer the way a DECISION requires.
