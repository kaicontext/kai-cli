...
A DEFECT NEEDS A TRIGGER THAT EXISTS TODAY. Every ISSUE names two things: a reachable trigger in the code as it stands after this change — an input, caller, configuration or state that actually reaches the line — and the failure mechanism, step by step, from that trigger to the wrong result. "Dormant today, but breaks if retry-incrementing is ever added to the other paths" and "a latent footgun for a future strict comparison" are not defects: their trigger is a change nobody has made (Cal.com #14943 and #11059 on the 2026-09-24 benchmark). Neither is "would break if the guards were reordered". A claim that an external API, library or query form is invalid — "Prisma does not accept ANY(array)" — is a defect only once you have verified it (kai_web_search, or the library's own source in the tree); unverified, it is an open question for the prose, and it never goes in ISSUES.

ONE ROOT CAUSE, ONE ISSUE. When the same mistake shows up in several places — one helper's return shape misread by every caller, one missing guard copied into three handlers — write ONE ISSUE at the place that explains it best and list the others in the same bullet as "(also: path:line, path:line)". Several bullets for one cause read as several problems and bury the rest; Cal.com #11059 on the 2026-09-24 benchmark got three findings for one refreshOAuthTokens return-shape mismatch (HubSpot, Zoho Bigin, Zoho CRM). A bullet that begins "same …" is a location, not an issue.
...
TRACE WHAT CROSSES A CALL. Opening both files is not reading the contract between them. For every value this change hands from one function to another — an id, a key, an argument, a token, a return value — write down both sides before you judge either: what the producer returns or creates, under which identifier, and what the consumer does with it. Check that they agree on the KIND of identifier (an internal id vs a client id vs a name), on each argument's meaning in its position (a credential id passed where a user id is expected), on the RETURN SHAPE (a fetch Response vs the token object inside it, a {success, data} wrapper vs its data), and on FRESHNESS (after a refresh, is the new token used, or the copy read before it?). On the 2026-09-24 benchmark every one of these was missed in a file the review said it had opened: Keycloak #36880 looked a per-client resource up by client.getId() while getOrCreateResource keyed it by resourceServer.getClientId(), and returned resource names where callers passed them to realm.getClientById; Keycloak #37038 returned resource ids where callers expected group ids; Cal.com #11059 passed credentialId where refreshOAuthTokens expected userId, read res.data off a fetch Response, and built a jsforce connection from the token it had just replaced. A mismatch you have traced is a defect; name both ends.

TWO REQUESTS AT ONCE. For every read, check and write of shared state — a database row, a counter, a one-time code, a balance, a status — run it twice at the same time in your head: both requests read the same value, both pass the check, both write. A counter written as the value read plus one loses an increment (Cal.com #14943: retryCount: reminder.retryCount + 1); a one-time code checked in memory and written back can be spent twice (Cal.com #10600: two logins with the same backup code both pass before either removes it); a status checked and then updated lets two workers take the same job. Each is a defect unless something serializes it: an atomic update (increment, compare-and-set, UPDATE … WHERE the old value), a transaction holding a row lock, or a unique constraint. Name the two requests and the interleaving; "not thread-safe" without them is not a finding.
...
TESTS, INTENT AND CONSISTENCY ARE NOT DEFECTS BY THEMSELVES. "This path has no test", "the test never asserts X", "this differs from the other handlers", a name, a missing method guard nothing reaches, and a behaviour change the PR says it makes on purpose are not ISSUES. Report one only when it names a specific missing requirement — a behaviour this change must guarantee (usually the fix it claims) that nothing verifies — or a concrete regression risk: what breaks, for whom, on which input. A change that claims to fix a bug and adds nothing that would fail without the fix has exactly such a missing requirement: that stays a finding. Otherwise leave it out, or say it in one sentence of prose. On the 2026-09-24 benchmark these took over the defect list: Keycloak #36880 ("the test never asserts an admin with MANAGE_CLIENTS is still allowed"), Cal.com #11059 ("inconsistent with every other handler in apps/web/pages/api"), Cal.com #10600 ("minLength/maxLength are native attributes, not zod validation"). An intentional change the author described is at most a DECISION, and only when it reaches a customer the way a DECISION requires.
