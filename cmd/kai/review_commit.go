...
// reviewer's raw closing text whenever the verdict is unreadable (the
// MatchUnknown branch in runReviewCommit). Add a spelling when a real run
...
// synonym is.
//
// Do NOT widen this to scan every word. The loop this replaced returned on its
// first iteration, which read as a scan and invited exactly that change:
// "diverges — the code no longer matches the stated intent" names two verdicts
// and means one, so scanning resolves it to whichever appears first in the map's
// iteration order. TestRCParseReviewOutput_UnreadableVerdictStaysUnknown pins
// the rule via its "negated" cases.
func rcVerdictFromValue(v string) (finding.Match, bool) {
	words := rcVerdictWords(v)
	if len(words) == 0 {
		return "", false
	}
	m, ok := rcVerdictSynonyms[words[0]]
	return m, ok
}
...
func rcParseReviewOutput(raw string) (risks []string, match finding.Match, note string) {
	...
	for _, line := range strings.Split(raw, "\n") {
		...
		case key == "FINDINGS:":
			...
			//
			// A second header supersedes the previous block ENTIRELY — verdict and
			// note included, not just the bullets. Resetting only the list left
			// "the last one is the real one" true for findings and false for the
			// verdict: a quoted block's "INTENT_MATCH: diverges" stayed in `stated`,
			// so the real block's "verified" read as two verdicts that disagree and
			// resolved to unknown — the exact amber this command exists to stop, and
			// produced by a verdict the reviewer never gave. The same leak recorded a
			// quoted NOTE as the assessment when the real block omitted one.
			//
			// Gated on a header already having been seen, because the reset is what
			// supersession means and nothing is superseded before the first header.
			// Ungated, ordering drift loses a verdict that was stated plainly:
			// "INTENT_MATCH: verified" ABOVE the block is the reviewer's only verdict,
			// and clearing it there would turn a readable review into unknown to fix
			// a leak that cannot have happened yet.
			if seenFindings {
				stated, conflict, note = "", false, ""
			}
			seenFindings = true
			inFindings = true
			risks = nil
