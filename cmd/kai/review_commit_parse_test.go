...
func TestRCParseReviewOutput_ProseNeverBecomesAVerdict(t *testing.T) {
	tests := []struct {
		...
		{"prose naming a verdict word twice",
			"The tests partially cover this.\nThe implementation matches the docs.\nNOTE: fine"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, match, _ := rcParseReviewOutput(tt.raw)
...
// A second FINDINGS: header supersedes the whole previous block, not only its
// bullets. The quoted block this command's own test fixtures invite a reviewer to
// copy carries an INTENT_MATCH line and a NOTE as well, and leaving those in place
// while restarting the list made "the last one is the real one" true for findings
// and false for the verdict: the quoted verdict survived, disagreed with the real
// one, and the disagreement resolved to unknown — an amber badge produced by a
// verdict the reviewer never gave.
func TestRCParseReviewOutput_SecondFindingsHeaderSupersedesTheWholeBlock(t *testing.T) {
	t.Run("quoted verdict does not poison the real one", func(t *testing.T) {
		raw := strings.Join([]string{
			"The fixtures in the test file look like:",
			"FINDINGS:",
			"- [bug] a.go:1 — quoted example",
			"INTENT_MATCH: diverges",
			"NOTE: quoted assessment",
			"",
			"My actual review:",
			"FINDINGS:",
			"- [correctness] review_commit.go:443 — the real defect",
			"INTENT_MATCH: verified",
			"NOTE: the real assessment",
		}, "\n")
		risks, match, note := rcParseReviewOutput(raw)
		want := []string{"[correctness] review_commit.go:443 — the real defect"}
		if !slices.Equal(risks, want) {
			t.Errorf("risks = %q, want %q", risks, want)
		}
		if match != finding.MatchVerified {
			t.Errorf("match = %q, want verified — the quoted verdict poisoned the real one", match)
		}
		if note != "the real assessment" {
			t.Errorf("note = %q, want the real block's note", note)
		}
	})

	// The reset cuts both ways: a verdict and note the reviewer only ever QUOTED
	// must not be attributed to it when the real block states neither. A
	// confidently wrong verdict is worse than an honest unknown.
	t.Run("quoted verdict is not inherited by a block that states none", func(t *testing.T) {
		raw := strings.Join([]string{
			"For example:",
			"FINDINGS:",
			"- [bug] a.go:1 — quoted example",
			...
			"",
			"FINDINGS:",
			"- [correctness] b.go:2 — the real defect",
		}, "\n")
		_, match, note := rcParseReviewOutput(raw)
		if match != finding.MatchUnknown {
			t.Errorf("match = %q, want unknown — a quoted verdict was recorded as the reviewer's own", match)
		}
		if note != "" {
			t.Errorf("note = %q, want empty — a quoted note was recorded as the assessment", note)
		}
	})

	// Ordering drift is not supersession. A verdict stated ABOVE the block is the
	// reviewer's only verdict, and the first header must not clear it — that would
	// lose a plainly readable verdict to fix a leak that cannot have happened yet.
	t.Run("a verdict above the first header survives", func(t *testing.T) {
		raw := strings.Join([]string{
			"INTENT_MATCH: verified",
			"NOTE: stated before the block",
			"FINDINGS:",
			"- [bug] a.go:1 — x",
		}, "\n")
		_, match, note := rcParseReviewOutput(raw)
		if match != finding.MatchVerified {
			t.Errorf("match = %q, want verified — the first header cleared a verdict it did not supersede", match)
		}
		if note != "stated before the block" {
			t.Errorf("note = %q, want the note stated above the block", note)
		}
	})
}
...
// unreadable-verdict log shows a real run producing a spelling we rejected.
